...
interface Props<Item extends InspectDetails> {
  inspects: Item[];
}

function ListItem<Item extends InspectDetails>({ id, datetime }: PropsWithChildren<Item>) {
  return (
    <li key={id}>
        ...
        <a className="block hover:bg-gray-50">
          <div className="px-4 py-4 sm:px-6">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-indigo-600 truncate">Daily Vehicle Inspection</p>
...
export function DailyInspectList<Item extends InspectDetails>({ inspects }: PropsWithChildren<Props<Item>>) {
  return (
